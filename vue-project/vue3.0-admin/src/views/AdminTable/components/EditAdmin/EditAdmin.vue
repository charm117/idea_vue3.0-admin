  ...
  <el-dialog title="编辑账号" :visible.sync="dialogFormVisible" v-dialogDrag width="25%">
    <el-form :model="form">
      ...
      <el-form-item label="地区选择" :label-width="formLabelWidth">
        <el-select name="province" id="province" class="classify" v-on:change="indexSelect01" v-model="indexId">
          <el-option :value="选择一级菜单">选择一级菜单</el-option>
          <el-option :value="item.id" v-for="(item,index) in select01">{{item.name}}</el-option>  <!---->
        </el-select>
        <!--二级菜单-->
        <el-select name="city" id="city" class="classify mt10" v-model="indexId2">
          <el-option value="选择二级菜单">选择二级菜单</el-option>
          <el-option :value="k.id" v-for="k in select02">{{k.name}}</el-option>
        </el-select>
      </el-form-item>
    </el-form>
